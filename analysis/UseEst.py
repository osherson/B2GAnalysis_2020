...
for VAR in EstVars:
	...
	B_up = outF.Get("rB_up_data_"+VAR[0])
	B_down = outF.Get("rB_down_data_"+VAR[0])
	B_stat = outF.Get("rB_stat_data_"+VAR[0])
	Btt = outF.Get("rB_nom_ttbar_"+VAR[0])
	Btt_stat = outF.Get("rB_stat_ttbar_"+VAR[0])
	Btt_up = outF.Get("rB_up_ttbar_"+VAR[0])
	Btt_down = outF.Get("rB_down_ttbar_"+VAR[0])

	B.Add(Btt, -1.)
	B.Add(Att, 1.)
	B_up.Add(Btt_up, -1.)
	B_up.Add(Att, 1.)
	B_down.Add(Btt_down, -1.)
	B_down.Add(Att, 1.)
	...
	plot_sig = []
	for S in SIG:
		S_temp = outF.Get("rA_"+S[3]+"_"+VAR[0])
	...
	ttLeg.Draw("same")
	C_tttemp.Print("../results/"+NAME+"/TTTEMP_"+NAME+"_"+VAR[0]+".root")
	C_tttemp.Print("../results/"+NAME+"/TTTEMP_"+NAME+"_"+VAR[0]+".png")
