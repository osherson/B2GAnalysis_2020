...
if not Blind:
	for Sigs in SIG:
		for VAR in EstVars2D:
			...
			rDATA = convertAsymGraph(rrDATA, rBKG_PREFIT, "data_"+cardname)

			for axis in [0,1]:

				DATA = Reroll(rDATA, VAR)[axis]
				BKG_PREFIT = Reroll(rBKG_PREFIT, VAR)[axis]
				BKG_PRETT = Reroll(rBKG_PRETT, VAR)[axis]
				SIGNAL_PREFIT = Reroll(rSIGNAL_PREFIT, VAR)[axis]
				BKG_S = Reroll(rBKG_S, VAR)[axis]
				BKG_STT = Reroll(rBKG_STT, VAR)[axis]
			...
			for axis in [0,1]:
				...
				BKG_S = Reroll(rBKG_S, VAR)[axis]
				BKG_STT = Reroll(rBKG_STT, VAR)[axis]
				SIGNAL = Reroll(rSIGNAL, VAR)[axis]
